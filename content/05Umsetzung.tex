...
\section{Hardware} % vielleicht überflüssig da alles in Konzept genannt
...
\section{Software}
    ...
    Die Magneten werden so weit außen am Rad wie möglich angebracht, um möglichst nah an den Radumfang zu kommen und eine genauere Messung der Geschwindigkeit zu ermöglichen.
    ...
    Eine Notbremsung wird durchgeführt, wenn ein Hindernis eine potenzielle Kollisionsgefahr darstellt. Die gemessene Entfernung zu diesem Hindernis wird unter Berücksichtigung der aktuellen Geschwindigkeit des Fahrzeugs ausgewertet. Dadurch kann ermittelt werden, ob eine Kollision wahrscheinlich ist. \\
        ...
        Die Entfernung zu einem Hindernis wird durch die Ultraschallsensoren gemessen. Dabei wird die Zeit gemessen, die ein Ultraschallsignal benötigt, um von einem Sensor ausgesendet zu werden, auf ein Hindernis zu treffen und zum anderen Sensor zurückzukehren. Anhand dieser Zeit und der Schallgeschwindigkeit kann die Entfernung zum Hindernis berechnet werden. \\
        Die Schallgeschwindigkeit beträgt etwa 343 Meter pro Sekunde. Für diesen Anwendungsfall kann dieser Wert jedoch in 34300 Zentimeter pro Sekunde umgerechnet werden, um die Berechnung der Entfernung in Zentimetern zu erleichtern. Die Formel zur Berechnung der Entfernung \( d \) in Zentimetern lautet: \\
            \begin{equation}
                d = \frac{t \cdot c}{2}
            \end{equation}
        wobei \( t \) die gemessene Zeit in Sekunden und \( c \) die Schallgeschwindigkeit in Zentimetern pro Sekunde ist. Die Division durch 2 erfolgt, da das Signal den Weg zum Hindernis und wieder zum Sensor zurücklegt. \\
    
        \subsubsection{Anhaltewegberechnung}
        Wie auf die gemessene Entfernung reagiert werden soll, wird durch die Berechnung des Bremswegs bestimmt, der anhand der aktuellen Geschwindigkeit berechnet wird. Hierzu wird die Formel für eine Notbremsung herangezogen. Die Formel für die Berechnung des Bremswegs bei einer normalen Bremsung kann in diesem Fall vernachlässigt werden, da es sich um die Implementierung einer Notbremsung handelt. Für eine Notbremsung wird folgende Formel verwendet: \\
        \begin{equation}
        ...
        \subsubsection{Bremsvorgang einleiten}
        Da das Fahrzeug nicht direkt an der Wand stehenbleiben sollte, sondern ein gewisser Sicherheitsabstand eingehalten werden soll, wird die Notbremsung bereits ausgelöst, wenn die gemessene Entfernung zum Hindernis in etwa 20 cm größer ist als der berechnete Bremsweg. Dadurch wird sichergestellt, dass das Fahrzeug rechtzeitig zum Stillstand kommt, bevor es das Hindernis erreicht. \\
        Ist diese Schwelle erreicht werden die Motoren sofort gestoppt % erklären was genau gemacht wird für bremsen
